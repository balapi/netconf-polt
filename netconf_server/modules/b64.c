 ...
 * \return BCMOS_TRUE if c is a base64 value.
 */
static bcmos_bool is_base64(unsigned char c)
{
 ...
 * buffer.
 * This function counts the number of valid base64 characters,
 ...
 * \return BCMOS_TRUE if the end of buffer was reached.
 */
static bcmos_bool extract_4bytes( const uint8_t** iterPos, const uint8_t* endPos,
                            uint8_t arr4[4], uint32_t* numExtracted )
{
    const uint8_t* iter=*iterPos;
    uint32_t validBytes = 0;
    bcmos_bool endReached = BCMOS_FALSE;
    ...
    while ( iter < endPos && validBytes<4 && !endReached )
    {
        ...
        else
        {
            ...
            if ( *iter != '=' )
            {
                NC_LOG_WARN("b64_decode() encountered invalid character(%c), "
...
// ---------------------------------------------------------------------------|
static bcmos_bool extract_3bytes( const uint8_t** iterPos, const uint8_t* endPos,
...
// ---------------------------------------------------------------------------|
static bcmos_bool encode_3bytes( const uint8_t** iterPos, const uint8_t* endPos,
                                 uint8_t** outPos )
{
    uint8_t arr3[3];
    uint32_t numBytes;

    bcmos_bool endReached = extract_3bytes( iterPos, endPos, arr3, &numBytes );
...
// ---------------------------------------------------------------------------|
bcmos_errno b64_encode ( const uint8_t* inbuff, uint32_t inbufflen,
...
// ---------------------------------------------------------------------------|
bcmos_errno b64_decode ( const uint8_t* inbuff, uint32_t inbufflen,
                            uint8_t* outbuff, uint32_t outbufflen,
                            uint32_t* retlen )
{
    uint8_t arr4[4];
