...
static bcmos_errno polt_cli_onu_add(bcmcli_session *session, const bcmcli_cmd_parm parm[], uint16_t nparms)
{
    ...
    const char *v_ani_name = bcmcli_parm_is_set(session, &parm[8]) ? parm[8].value.string : NULL;
    bcmos_errno rc;
    ...
    rc = sim_tr451_vendor_onu_added(cterm_name, v_ani_name, onu_id, &serial_number, registration_id, flags,
...
bcmos_errno tr451_vendor_cli_init(bcmcli_entry *dir)
{
    ...
    /* Add ONU */
    {
        static bcmcli_cmd_parm cmd_parms[] = {
            ...
            BCMCLI_MAKE_PARM("v_ani", "v_ani name. Implies 'expected' flag", BCMCLI_PARM_STRING, BCMCLI_PARM_FLAG_OPTIONAL),
            { 0 }
        } ;
