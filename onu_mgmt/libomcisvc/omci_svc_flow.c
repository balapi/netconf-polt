...
bcmos_errno omci_svc_flow_clear(bcmonu_mgmt_flow_cfg *flow, bcmonu_mgmt_complete_cb cb, void *context)
{
    ...
    if (rc)
    {
        if (!omci_svc_is_issu)
        {
            /** @note issu in stub mode just returns OK, and prints the standard log */
            /* ignore onu key validation error */
            return rc;
        }
    }
    ...
    if (NULL == flow_cfg_entry)
    {
        ...
        {
            /** @note issu in stub mode just returns OK, and prints the standard log */
            OMCI_SVC_LOG(INFO, olt_id, onu_key, &flow->hdr.hdr, "issu stub mode: flow entry Clear success\n");
            return BCM_ERR_OK;
        }
        else
        {
